function nextQuostion( curQuo ) {
	...
	switch( currentQuostion ) {
		...
		case '6-2': // Четвертый вопрос по шкафам
			/* Проверяем что выбран тип шкафа */
			if ( ( document.getElementById( 'answer-6-2-1' ).checked == false ) && ( document.getElementById( 'answer-6-2-2' ).checked == false ) && ( document.getElementById( 'answer-6-2-3' ).checked == false ) && ( document.getElementById( 'answer-6-2-4' ).checked == false ) ) {
				alert( 'Для продолжения выберите желаемый подарок' );
				return false;
			}
